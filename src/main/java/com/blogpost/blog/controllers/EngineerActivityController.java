...
public ResponseEntity<?> getAllEngineerTicket(HttpSession session) {
    try {
        ...
        if (username == null || company == null || groupName == null) {
            return new ResponseEntity<>("Session attributes missing. Please log in again.", HttpStatus.UNAUTHORIZED);
        }

        // Fetch tickets using the repository method
        List<Ticket> findTicketsByEngineerDetails = ticketRepo.findTicketsByEngineerDetails(groupName, company, username);
